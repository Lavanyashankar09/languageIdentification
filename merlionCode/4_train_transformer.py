...
class LanguageIdentificationModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, dropout_rate=0.5, num_heads=8, num_encoder_layers=6):
        super(LanguageIdentificationModel, self).__init__()
        
        # Define the Transformer architecture
        self.embedding = nn.Linear(input_dim, hidden_dim)  # Input projection
    ...
    def positional_encoding(self, seq_len, hidden_dim):
        """Generate positional encoding for the given sequence length."""
    ...
    def forward(self, x):
        ...
        # Apply embedding layer (input projection)
        x = self.embedding(x)  # Shape: (batch_size, seq_len, hidden_dim)
        
        # Generate dynamic positional encoding
        pos_enc = self.positional_encoding(seq_len, x.size(-1)).to(x.device)  # Shape: (1, seq_len, hidden_dim)
        ...
        # Permute to match the expected input format for Transformer (seq_len, batch_size, hidden_dim)
