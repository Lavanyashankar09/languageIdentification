...
class LanguageIdentificationModel(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, num_classes=2, dropout_rate=0.5):
        ...
        self.bilstm2 = nn.LSTM(
            input_size=hidden_dim * 2,  # *2 because of bidirectional
            hidden_size=256,
            batch_first=True,
            bidirectional=True,
            dropout=dropout_rate
        ...
        self.fc_layers = nn.Sequential(
            nn.Linear(512, 128),  # 512 = 256*2 (bidirectional output)
            nn.ReLU(),
            nn.Dropout(dropout_rate),
    ...
    def forward(self, x):
        ...
        # Transpose for Conv1D layers (batch_size, input_dim, sequence_length)
        x = x.transpose(1, 2)
        ...
        # Transpose back for LSTM layers (batch_size, sequence_length, channels)
        x = x.transpose(1, 2)
    ...
    def init_weights(self):
        """Initialize weights with appropriate methods for different layer types"""
        for name, param in self.named_parameters():
            if 'weight' in name:
                if 'conv' in name and len(param.shape) > 1:
                    # Convolutional layers
                    nn.init.kaiming_normal_(param, mode='fan_out', nonlinearity='relu')
                elif 'lstm' in name and len(param.shape) > 1:
                    # LSTM layers
                    nn.init.orthogonal_(param)
                elif 'fc' in name and len(param.shape) > 1:
                    # Fully connected layers
                    nn.init.xavier_normal_(param)
                else:
                    # For 1D weights (e.g., vectors), use uniform initialization
...
def load_checkpoint(model, optimizer, checkpoint_path, device='cpu'):
    if os.path.isfile(checkpoint_path):
        ...
        print(f"Checkpoint loaded from {checkpoint_path}, starting from epoch {epoch}, "
              f"Validation Accuracy: {val_accuracy:.4f}, Validation Loss: {val_loss:.4f if val_loss is not None else 'N/A'}, "
              f"Best Val Accuracy: {best_val_accuracy:.4f}, Learning Rate: {learning_rate:.4e if learning_rate is not None else 'N/A'}")
...
def test_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            _, predicted = torch.max(outputs.data, 1)  # Predicted class labels
            y_pred.extend(predicted.tolist())
            
            probabilities = torch.softmax(outputs, dim=1)[:, 1]  # Probabilities for class 1 (for binary classification)
    ...
    eer_index = np.argmin(difference)  # EER is the index where the difference between FPR and FNR is smallest
    eer = (fpr[eer_index] + fnr[eer_index]) / 2.0  # Equal Error Rate (EER)
    ...
    # Returning all relevant metrics as a dictionary
    results = {
        'y_true': y_true,
        'y_pred': y_pred,
        'y_scores': y_scores,
...
def main(hdf5_dir, save_dir_plot, save_dir_cp, layer, hidden_dim, batch_size, num_epochs, patience, save_every_n_epochs, checkpoint_path=None):
    ...
    # extra
    model = LanguageIdentificationModel(input_dim=input_dim, hidden_dim=128, num_classes=2)
