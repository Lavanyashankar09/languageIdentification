...
class AudioSegmenter:
    ...
    def segment_audio_files(self, df):
        ...
        for index, row in df.iterrows():
            ...
            # Save the segment
            output_file = os.path.join(self.output_folder, f"{output_file_prefix}_segment_{index}.wav")
            try:
...
def main():
    ...
    output_path = "/export/c09/lavanya/languageIdentification/"
    ...
    output_folder = os.path.join(output_path, "segments")
    output_pickle_path = os.path.join(output_path, "pickles", "segments.pkl")
